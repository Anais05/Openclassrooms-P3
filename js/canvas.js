...
var Signature = 
{
    ...
    estVide : true, // pour rendre le canvas obligatoire 
    ...
    // Méthode qui récupére les coordonnées de l'Élément de pointage souris ou doigt
    ...
    {
        sourisPosition = this.getMousePos(event); //  "getMousePos"retourne les coordonnées de l'élemnt de pointage
        positionX = sourisPosition.x;
        positionY = sourisPosition.y;
        this.drawing(positionX, positionY);
    },

    // Méthode qui permet de dessiner dans le canvas
    drawing : function(positionX, positionY) 
    {
...
Signature.canvas.addEventListener("mousedown", Signature.activateDraw.bind(Signature));
Signature.canvas.addEventListener("mousemove", Signature.moveMouse.bind(Signature));
Signature.canvas.addEventListener("mouseup", Signature.disableDraw.bind(Signature));
